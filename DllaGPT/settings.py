...
def parse_args():
    ...
    parser.add_argument("--data_dir", type=str, default="C:/Users/Eric/PycharmProjects/ELLAM/Data_Directory") #, required=True
    ...
    parser.add_argument("--gen_lm_sample_percentage", type=float, default=0.30)
    ...
    parser.add_argument("--model_dir_root", type=str, default="../Model_Directory") #, required=True
    ...
    args.device_ids = GPUtil.getAvailable(maxLoad=1, maxMemory=1, limit=args.n_gpus)
    ...
    # 当需要create extra data的时候启动
    ### 强行赋值代码仅供测试的时候使用####
    # args.memory_sizes = [2048.0]
    # args.n_gpus = 1
    # args.device_ids = [0]


    print("args.device_ids:", args.device_ids)
    print("args.n_gpus:", args.n_gpus)
    args.memory_sizes = [gpus[device_id].memoryTotal for device_id in args.device_ids]
    print("args.memory_sizes:", args.memory_sizes)




    # args.memory_sizes[0] = args.memory_sizes[0] * (1 - 0.04 * (args.n_gpus-1))
    # print("args.memory_sizes[0]:", args.memory_sizes[0])
    ...
    special_tokens = {"ans_token":'__ans__', "pad_token":'__pad__', "unk_token":'__unk__', "eos_token": '<|endoftext|>', "sep_token": '__sep__'}
    if args.use_sep:
    ...
    with open(data_attrs_path, "r", encoding='utf-8') as f:
        data_attrs = json.load(f)

    if args.seq_train_type == "multitask":
        args.n_train_epochs = {'_'.join(args.domains): args.n_train_epochs}
    elif args.unbound:
        pass
    else:
        ...
        if args.dynamic_epochs:
            ...
            args.n_train_epochs = {domain: args.n_train_epochs for domain in args.domains}
...
DOMAIN_DICT = {
    "tweets":{
        ...
        "test":os.path.join(args.data_dir,"tweets-test.json"),
    ...
    "restaurant":{
        ...
        "test":os.path.join(args.data_dir,"restaurant-test.json"),
        # "n_train_epochs": args.n_train_epochs
        "n_train_epochs": 5
    },
    "finance":{
        "train":os.path.join(args.data_dir,"finance-train.json"),
        "eval":os.path.join(args.data_dir,"finance-val.json"),
        "test":os.path.join(args.data_dir,"finance-test.json"),
        # "n_train_epochs": args.n_train_epochs
        "n_train_epochs": 2
    },
    "laptop-multi": {
        "train": os.path.join(args.data_dir, "laptop-multi-train.json"),
        "eval": os.path.join(args.data_dir, "laptop-multi-val.json"),
        "test": os.path.join(args.data_dir, "laptop-multi-test.json"),
        # "n_train_epochs": args.n_train_epochs
        "n_train_epochs": 5
    },
    "restaurant-multi": {
        "train": os.path.join(args.data_dir, "restaurant-multi-train.json"),
        "eval": os.path.join(args.data_dir, "restaurant-multi-val.json"),
        "test": os.path.join(args.data_dir, "restaurant-multi-test.json"),
        # "n_train_epochs": args.n_train_epochs
        "n_train_epochs": 5
    }
}
