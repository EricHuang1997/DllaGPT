from utils import create_dataloader, QADataset
from settings import args,  TASK_DICT
    ...
    for t in ["train", "eval", "test"]:
        for task in TASK_DICT.keys():
            ...
            if os.path.exists(pkl_path) and not redo:
                continue
            # This is the real data preprocessing part（serialized part）
            dataset = QADataset(data_path, t)
            with open(pkl_path, "wb", encoding='utf-8') as f:
                pkl.dump(dataset,f)
...
def dump_data_attrs(task):
    ...
    for t in ["train", "eval", "test"]:
        ...
        data_path = TASK_DICT[task][t]
        pkl_path = re.sub("json","pkl", data_path)
        with open(pkl_path, "rb", encoding='utf-8') as f:
            dataset = pkl.load(f)
...
def parallel_dump_data_attrs(tasks=TASK_DICT.keys()):
    ...
    for a in attrs: 
        attr_dict.update(a)
    with open("data_attrs.json","w", encoding='utf-8') as f:
