...
def train(task_ids, model):
    ...
    if len(TOKENIZER) != TOKENS_WEIGHT.shape[0]:
        TOKENS_WEIGHT = torch.cat((TOKENS_WEIGHT, torch.ones([1]).to.cuda()))
...
if __name__ == '__main__':
    ...
    model = None
    if args.seq_train_type == "cross_domain":
        model = train(list(range(len(args.domains))), model)
    else:
        if args.unbound:
