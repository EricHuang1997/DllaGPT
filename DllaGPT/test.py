...
def test_one_to_one(domain_load, domain_eval, model, score_dict):
    ...
    logger.info("mean_accuracy: { domain: %s, mean_accuracy: %s }" % (domain_eval, total_accuracy / len(qa_results)))
...
def test_one_to_many(domain_load):
    ...
    for ep in range(args.n_train_epochs[domain_load]):
        ...
        with torch.no_grad():
            ...
            test_one_to_one(domain_load, domain_load, model, score_dict)
